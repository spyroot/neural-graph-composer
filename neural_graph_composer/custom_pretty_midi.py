"""
This class contains number of fixes to original implementation.
...
import numpy as np
import six
from pretty_midi import pretty_midi, functools, qpm_to_bpm, key_name_to_key_number
from pretty_midi import ControlChange
from pretty_midi import PitchBend
...
class CustomPrettyMIDI(pretty_midi.PrettyMIDI):
    ...
    def __init__(self,
                 ...
                 initial_tempo: Optional[float] = 120.,
                 drum_channel: Optional[int] = 9,
                 is_debug: Optional[bool] = False):
        """
        Initialize either by populating it with MIDI data from a file or
        from scratch with no data.
        """
        # Mustafa TODO push to github to main PrettyMIDI
    ...
    def _note_off_event(self, event, last_note_on, __get_instrument, current_instrument, track_idx):
        ...
        for start_tick, velocity in notes_to_close:
            ...
            ints = __get_instrument(program, event.channel, track_idx, 1)
            # Add the note event
            ints.notes.append(note)
    ...
    def _load_instruments(self, midi_data):
        ...
        def __get_instrument(_program, _channel, _track, create_new):
            """
            ...
            if (_program, _channel, _track) in instrument_map:
                return instrument_map[(_program, _channel, _track)]
            # If there's a straggler instrument for this instrument and we
            # aren't being requested to create a new instrument
            if not create_new and (_channel, _track) in stragglers:
                return stragglers[(_channel, _track)]
            # If we are told to, create a new instrument and store it
            if create_new:
                is_drum = (_channel == self.drum_channel)
                _instrument = Instrument(_program, is_drum, track_name_map[track_idx])
                # If any events appeared for this instrument before now,
                # include them in the new instrument
                if (_channel, _track) in stragglers:
                    straggler = stragglers[(_channel, _track)]
                    _instrument.control_changes = straggler.control_changes
                    _instrument.pitch_bends = straggler.pitch_bends
                # Add the instrument to the instrument map
                instrument_map[(_program, _channel, _track)] = _instrument
            ...
            # instrument
                ...
                _instrument = Instrument(program, track_name_map[track_idx])
                ...
                stragglers[(_channel, _track)] = _instrument
            return _instrument
        ...
        for track_idx, track in enumerate(midi_data.tracks):
            ...
            for i, event in enumerate(track):
                ...
                # Store pitch bends
                    ...
                    ints = __get_instrument(program, event.channel, track_idx, 0)
                    # Add the pitch bend event
                    ints.pitch_bends.append(bend)
                # Store control changes
                    ...
                    ints.control_changes.append(control_change)
                elif event.type == 'end_of_track':
                    # close notes
                    self._finish_notes(event, last_note_on,
                                       __get_instrument, current_instrument, track_idx, end_of_seq=True)
                # else:
                # if self.is_debug:
                #     print("Unmatched even", event)
    ...
    def get_onsets(self):
        ...
        for ints in self.instruments:
            onsets = np.append(onsets, ints.get_onsets())
    ...
    def adjust_times(self, original_times: np.ndarray, new_times: np.ndarray):
        ...
        def adjust_events(event_getter):
            ...
            for _instrument in self.instruments:
                event_getter(_instrument).sort(key=lambda e: e.time)
            # Correct the events by interpolating
            event_times = np.array(
                [event.time for _instrument in self.instruments
                 for event in event_getter(_instrument)])
            adjusted_event_times = np.interp(
                event_times, original_times, new_times)
            for n, event in enumerate([event for _instrument in self.instruments
                                       for event in event_getter(_instrument)]):
            ...
            for _instrument in self.instruments:
                ...
                # new_times[0]
                valid_events = [event for event in event_getter(_instrument)
    ...
    def serialize(self, mido_file=None):
        """
         It add additional method without changing state parent class.
         It return MIDO file before it serialized ot file.
         So we avoid any IO if that required.
        :param mido_file:
        :return:
        """
        ...
        def event_compare(a, b):
            ...
            :param a:  mido Message
            :param b:  mido Message
            :return:
            """
            secondary_sort = {
                ...
                'control_change': lambda e: ((7 * 256 * 256) + (e.control * 256) + e.value),
            ...
            if (a.time == b.time and
                    a.type in secondary_sort and
                    b.type in secondary_sort):
                return (secondary_sort[a.type](a) -
                        secondary_sort[b.type](b))
            # Otherwise, just return the difference of their ticks.
            return a.time - b.time

        #  TODO added in case we want pass MIDI file
        #  and use mido file abstraction to send live.
        if mido_file is None:
            mido_file = mido.MidiFile(ticks_per_beat=self.resolution)
        else:
            mido_file.ticks_per_beat = self.resolution
        ...
        for l in self.lyrics:
            timing_track.append(mido.MetaMessage('lyrics',
                                                 time=self.time_to_tick(l.time),
                                                 text=l.text))

        # Sort the (absolute-tick-timed) events.
        timing_track.sort(key=functools.cmp_to_key(event_compare))
        # Add in an end of track event
        timing_track.append(mido.MetaMessage('end_of_track',
                                             time=timing_track[-1].time + 1))

        mido_file.tracks.append(timing_track)
        ...
        channels.remove(self.drum_channel)
        for n, instrument in enumerate(self.instruments):
            ...
            channel = self.drum_channel if instrument.is_drum else channels[n % len(channels)]
            ...
            for control_change in instrument.control_changes:
                track.append(mido.Message(
                    'control_change', time=self.time_to_tick(control_change.time),
            ...
            for i, (a, b) in enumerate(zip(track[:-1], track[1:])):
                if (a.time == b.time and
                        a.type == 'note_on' and
                        b.type == 'note_on' and
                        a.note == b.note and
