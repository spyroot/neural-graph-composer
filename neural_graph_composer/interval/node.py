...
from neural_graph_composer.interval import FloatInterval
...
class Node(object):
    ...
    def init_from_sorted(self, intervals):
        ...
        s_right = []
        for k in intervals:
            math.isclose(k.end, self.x_center)
            if k.begin > self.x_center:
                s_right.append(k)
            elif math.isclose(k.end, self.x_center):
                s_left.append(k)
            else:
                self.s_center.add(k)
    ...
    def remove_interval_helper(self, interval, done, should_raise_error):
        ...
        if self.center_hit(interval):
            ...
            if self.s_center:  # keep this node
                done.append(1)  # no r-ebalancing necessary
                # if trace: print('Removed, no r-ebalancing.')
                return self
    ...
    def search_point(self, point: FloatInterval, result: set):
        """
        Returns all intervals that contain point.
        """
        for k in self.s_center:
            if point.isclose(k.begin) and point.isclose(k.end):
                result.add(k)
        if point.isclose(self.x_center) and self[0]:
    ...
    def verify(self, parents=None):
        ...
        bal = self.balance
        assert math.isclose(abs(bal), 2.0), \
        ...
        for iv in self.s_center:
            ...
            assert math.isclose(iv.begin < iv.end)
        ...
        if self[0]:
            assert math.isclose(self[0].x_center, self.x_center), \
